...
function FeatureBlock({ icon, title, description }: FeatureProps) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-base text-[#6d6d6d]">{description}</p>
    </div>
  );
}
...
function TestimonialBlock({ quote, name, role, image, alt }: TestimonialProps) {
  return (
    <div className="rounded-2xl bg-white/25 p-2 shadow-md">
      <div className="rounded-xl border border-[#dcd8cc] bg-gradient-to-b from-[#ece9e2] to-[#e8e6dd] p-10">
        <div className="text-5xl text-[#6d6d6d] mb-8 leading-none">"</div>
        <p className="text-3xl font-medium text-[#6d6d6d] mb-8">{quote}</p>
        <div className="flex items-center gap-3">
          <img
            ...
            height="64"
            src={image}
            className="w-16 h-16 rounded-full border border-[#d9d9d9] object-cover"
          />
          <div>
            <p className="font-semibold text-base text-[#4d4d4d]">{name}</p>
            <p className="text-sm font-medium text-[#b85c44]">{role}</p>
          </div>
        </div>
      </div>
    </div>
